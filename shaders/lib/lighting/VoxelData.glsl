//================================================================================================//
// Voxel Data — 体素追踪共享工具（照抄 ITRP TracingUtilities.glsl / TracingNoise.glsl）
//
// - VoxelRandUnitVector / VoxelHemisphereUnitVector：均匀球面/半球采样（ITRP 语义，
//   pdf = saturate(dot(dir, normal)) * 2.0，见各调用点）
// - VoxelSphereHit / VoxelHitLightSphere：发射光球形光源（照抄 ITRP BlockShape 的
//   SphereIntersectionLength / HitLightShpere）：光线对准球心 → 1.0，擦边 → 平滑衰减。
//   光源体素在追踪/IRC 中走此路径（平滑贡献 + 穿透不挡光），普通固体才命中停止。
// - VoxelAlbedoToAbsorption / VoxelIsTranslucentAbsorb：透明吸收（照抄 ITRP
...
// 均匀球面采样（ITRP RandUnitVector：z 方向球坐标）
...
// 均匀半球采样（ITRP HemisphereUnitVector：均匀球面采样后按法线翻转到半空间）
...
// Pack2xU8 解码（照抄 ITRP Utilities.glsl Unpack2xU8_Y_from_U16 语义，L151-154）：
...
// 该方块写入时的真实 sky（ITRP 用 w 通道存 skylight 的原因，Shadow FSH 同款打包）。
// 用浮点取模代替位运算：与 ITRP 位运算版精度等价（rgba16f 的 half 存储是精度天花板，
...
// 全强度开关）。弦长按半径归一化 → 0~1 与 ITRP 的 r=0.5 弦长同尺度。
...
// 发射光颜色表（照抄 ITRP BlockShape.glsl LightShpereColor 思路，按材料 ID 取固定光源色）。
...
// 发射光球形光源贡献（照抄 ITRP HitLightShpere；光源色 = VoxelLightColor 固定色表，
// VOXEL_GI_BOOST 替代 ITRP 的 BLOCKLIGHT_BRIGHTNESS 旋钮）。
...
// 近场 1-2 格几乎不变（dist=1 → 87%）。ITRP 用低强度脉冲（×0.1）天然降方差，
...
// 透明吸收系数（照抄 ITRP Utilities.glsl AlbedoToAbsorption）：
...
// 透明吸收判定（照抄 ITRP isTranslucent：水3 / 玻璃4 / 树叶13）。
...
// 命中点精确图集 UV（照抄 ITRP GetAtlasCoord）。
...
// 方块形状求交（ITRP BlockShape 完整版，ID 平移 +150）：IsHitBox/HitShape/IsHitBlock
