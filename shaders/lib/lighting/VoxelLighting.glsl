...
// ------ Shadow Map 平铺布局（参考实现 VoxelProfile.glsl 移植）------
...
// 发射光球形光距离衰减（照抄 参考实现 语义的补充，2026-08-04 #8）：远场（16 格外）偶发
...
// 发射光球形光源半径（体素格数）。参考实现 用 0.5（格内切球），但那是多 SPP + ×0.1 低强度
...
// 追踪端发射光强度（参考实现 架构：发射光主体由 IRC 时域累积承载——IRC 注入端用
// VOXEL_GI_BOOST 强注入、时间混合 0.99 平滑；追踪端是每像素每帧 1 条随机光线的
// 高方差采样，发射光脉冲必须压低，否则贴光源面"命中/未命中"跳变 → 闪烁）。
// 参考实现 DiffuseTracing 里 HitLightShpere × BLOCKLIGHT_BRIGHTNESS=0.1（脉冲 0~0.1），
// 我们的追踪端脉冲 = 0.225~1.5（BOOST 1.5）→ 方差高 2 个数量级。此系数把追踪端
// 脉冲压到 ~0.07~0.45（仍高于 参考实现 的低配亮度），近场火把光主体依赖 IRC。
...
// 0.99 = 参考实现 默认（PT_IRC_BLENDWEIGHT 0.99），IRC 存"表面体素辐照度"变化慢，
...
// 新暴露格天空播种倍率（参考实现 PT_IRC_INITIAL_SKYLIGHT 思路）：相机移动时 64³ 前缘
...
// 2026-08-04 真阳光改造（照抄 参考实现 思路）：阳光注入主体改为"阴影贴图判定直射"（sunVis），
// vanilla 天空光 lightmap 降级为弱环境底，保留洞穴渐变。
#define VOXEL_GI_SUN_STRENGTH 0.5      // 真阳光直射注入倍率（× sunLight 暖阳色，2026-08-06 对齐 参考实现 sunLight 后提亮，让阳光反弹传播到阴影）
...
// NOLIGHT 兜底（参考实现 NOLIGHT_BRIGHTNESS 思路）：命中固体/出界外的闭塞处底光，
// 洞穴深处不黑死。参考实现 默认 7e-6 是物理尺度（白天物理辐照度 ~300），
...
// 追踪 GI 强度总旋钮：信号量级 = 追踪值 × STRENGTH（命中/出界已按 参考实现 语义全强度输出，
...
// 追踪端阳光弹射强度（参考实现 sunLight = colorShadowlight，无额外小系数；阴影贴图判定直射）。
...
// 追踪端出界天空（对齐 参考实现 SkyLighting 语义：出界 = skyColor × pdf × lightmap 衰减，无 0.05
// 小系数——参考实现 正是靠 lightmap 衰减防室内漏光，不是靠压低天空值）。
