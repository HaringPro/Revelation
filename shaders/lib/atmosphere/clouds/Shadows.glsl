...
float CalculateCloudShadows(in vec3 rayPos, in float dither) {
	...
	float invSteps = rcp(steps);
	float stepLength = (intersection.y - intersection.x) * invSteps;
	...
	// Pre‑compute constants to reduce per‑pixel ALU in the loop
	float invCumulusThickness = rcp(cumulusThickness);
	float extinctionCoeff = rLOG2 * cumulusExtinction;
	float oneMinusMinTransmittance = 1.0 - cloudMinTransmittance;
	...
	for (uint i = 0u; i < uint(steps) && extinction < threshold; ++i, rayPos += rayStep) {
		...
		float heightFraction = (length(rayPos) - cumulusBottomRadius) * invCumulusThickness;
	...
	float transmittance = exp2(-extinctionCoeff * extinction);
	transmittance = saturate((transmittance - cloudMinTransmittance) / oneMinusMinTransmittance);
