...
--------------------------------------------------------------------------------
	...
	Pass: Cloud CBR temporal upscaling & storing linear depth
...
/* RENDERTARGETS: 2,9 */
layout (location = 0) out float linearDepthOut;
...
void main() {
	...
	linearDepthOut = ScreenToViewDepth(depth);
	...
	if (depth > 0.999999) {
		...
		disocclusion = disocclusion || minOf(textureGather(colortex2, prevCoord, 0)) < far;
